...
resultSet = await connection.execute(`select count(*) as total_records from ac_item_uom where item_code = ${itemCode}`);
const total_records = resultSet.rows[0][0];
...
//Check item name exist start
let resultSet = await connection.execute(`select code from ac_item where lower(name)=lower('${item.name}') and code<> ${item.code}`);
if(resultSet.rows.length > 0) {
await connection.close();
throw `${item.name} already exists`;
}
...
if(trader.bank_account_ifsc.length >  20) {
...
async getAll(filters = { start_date: '', end_date: '', page: 0, limit: 0, is_deleted: 'N' }) {
...
queryStr = `select aci.code as invoice_number, customer_code, total_items, taxable_amt, igst_amt, cgst_amt, sgst_amt, total_amt, invoice_date, acc.name as customer_name from ac_invoice aci left join ac_customer acc on aci.customer_code = acc.code where invoice_date >= TO_DATE('${filters.start_date}', 'YYYY-MM-DD') AND invoice_date <= TO_DATE('${filters.end_date}', 'YYYY-MM-DD') AND aci.is_deleted IN (${filters.is_deleted}) order by invoice_number desc`;
}
else {
queryStr = `select aci.code as invoice_number, customer_code, total_items, taxable_amt, igst_amt, cgst_amt, sgst_amt, total_amt, invoice_date, acc.name as customer_name from ac_invoice aci left join ac_customer acc on aci.customer_code = acc.code where aci.is_deleted IN (${filters.is_deleted}) order by invoice_number desc`;
}
...
if(filters.start_date && filters.end_date) {
queryStr = `select count(*) as total_records from ac_invoice where invoice_date >= TO_DATE('${filters.start_date}', 'YYYY-MM-DD') AND invoice_date <= TO_DATE('${filters.end_date}', 'YYYY-MM-DD') AND is_deleted IN (${filters.is_deleted})`;
}
else {
queryStr = `select count(*) as total_records from ac_invoice where is_deleted IN (${filters.is_deleted})`;
}
...
let resultSet;
if(!filters.page && !filters.limit) {
resultSet = await connection.execute(`select acr.code as receipt_number,customer_code,amount,narration,receipt_date,acc.name as customer_name from ac_receipt acr left join ac_customer acc on acr.customer_code = acc.code where lower(acc.name) like lower('${filters.search ?? ""}%') order by receipt_date desc`);
}
else {
const offset = (filters.page - 1) * filters.limit;
resultSet = await connection.execute(`select acr.code as receipt_number,customer_code,amount,narration,receipt_date,acc.name as customer_name from ac_receipt acr left join ac_customer acc on acr.customer_code = acc.code where lower(acc.name) like lower('${filters.search ?? ""}%') order by receipt_date desc offset ${offset} rows fetch next ${filters.limit} rows only`);
}
...
resultSet = await connection.execute(`select count(*) as total_records from ac_receipt acr left join ac_customer acc on acr.customer_code = acc.code where lower(acc.name) like lower('${filters.search ?? ""}%')`);
const total_records = resultSet.rows[0][0];

await connection.close();
return { "data": receipts, "total_records": total_records };
...
resultSet = await connection.execute(`select count(*) as total_records from ac_customer_transactions acct left join ac_customer acc on acct.customer_code = acc.code where lower(acc.name) like lower('${filters.search ?? ""}%')`);
const total_records = resultSet.rows[0][0];
